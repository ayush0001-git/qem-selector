...
def main():
    ...
    for day, mult in enumerate(days_noise_multipliers, start=1):
        ...
        def mock_get_backend_info(b_name: str) -> dict:
            info = original_info.copy()
            info["avg_2q_error"] = simulated_2q_error
            # Readout error also drifts slightly
            info["avg_readout_error"] = original_info["avg_readout_error"] * (1.0 + (mult - 1.0) * 0.5)
            return info
